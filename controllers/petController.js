        ...
        nearByUsers.map(async (nearUser) => {
          if (nearUser.distance < 10) {
            ...
            const notification = new Notification(
              nearUser.user.userId,
              "Missing",
              `${petName} is missing in your area`,
              `${userDet.fullName} has lost its pets ${petName}. If you see this pet please inform.`,
              "false",
              new Date().toISOString()
            );
...
module.exports.deletePetPost = async (req, res) => {
  try {
    ...
    const [responseUser] = await query(
      sqlQueries.getUserById(responsePet.userId)
    );
...
module.exports.addToFavourites = async (req, res) => {
  try {
    ...
    const favouritePets = userRes.favouritePetsId
      ? [userRes.favouritePetsId, req.params.petId]
      : [req.params.petId];
    const response = await query(
      sqlQueries.updateUser(`favouritePetsId="${favouritePets}"`, req.userId)
    );
...
module.exports.removeFavourite = async (req, res) => {
  try {
    ...
    const newFavourites = userRes.favouritePetsId
      ...
      .filter((idx) => idx != req.params.petId);
    const response = await query(
      sqlQueries.updateUser(`favouritePetsId="${newFavourites}"`, req.userId)
    );
    return res.json(successMessage(response));
  } catch (error) {
    return res.status(400).send(error.message);
